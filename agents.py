...
class ActorCriticAgent(nn.Module):
    def __init__(self, conf, action_dim, device) -> None:
        ...
        if conf.Models.Agent.AC.Optimiser == 'Laprop':
            self.optimizer = LaProp(self.parameters(), lr=conf.Models.Agent.AC.Laprop.LearningRate,
                                    eps=conf.Models.Agent.AC.Laprop.Epsilon)
        ...
        self.lr_scheduler = torch.optim.lr_scheduler.LambdaLR(self.optimizer, lr_lambda=lambda
            step: 1.0)  # No lr schedule but neccessary for the warm up
    ...
    def update(self, latent, action, old_logits, context_latent, context_reward, context_termination, reward,
               termination, logger, global_step):
        '''
...
class PPOAgent(nn.Module):
    def __init__(self, conf, action_dim, device):
        ...
        if conf.Models.Agent.PPO.Optimiser == 'Laprop':
            self.optimizer = LaProp(self.parameters(), lr=conf.Models.Agent.PPO.Laprop.LearningRate,
                                    eps=conf.Models.Agent.PPO.Laprop.Epsilon)
